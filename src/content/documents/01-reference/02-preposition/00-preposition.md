...
A preposition matches a set of facts.
They are used in [`j.query`](../jinaga/query/) to return an array.
Or they are used in [`j.watch`](../jinaga/watch/) to update a user interface as things change.

Begin the chain by calling [`j.for`](./for/).
Pass in a template function, which describes the shape of facts to match.
This creates a [specification](./specification/).

Extend the chain by calling [`then`](./then/).
Pass in another template function that continues where the previous one left off.
